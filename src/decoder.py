...
class MultiHeadAttention(nn.Module):
    ...
    def __init__(self, d_model: int, num_attention_heads: int):
        ...
        self.heads = nn.ModuleList([AttentionHead(d_model,d_head,d_head,d_head) for _ in range(num_attention_heads)])
    ...
    def forward(self, hidden_state, mask=None):
        ...
        return self.output_linear(torch.cat(x, dim=-1))
...
class TransformerDecoderLayer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, mask: torch.Tensor) -> torch.Tensor:
        ...
        x = self.layer_norm_1(x + att)
        
        ff = self.feed_forward(x)
        x = self.layer_norm_2(x + ff)
        return x
...
class TransformerDecoder(nn.Module):
    ...
    def forward(self, input_ids: torch.Tensor) -> torch.Tensor:
        ...
        mask = mask.unsqueeze(0).expand(batch_size, seq_length, seq_length)
